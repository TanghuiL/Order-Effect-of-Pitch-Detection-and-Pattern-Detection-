...
prompt = {'Epoch starts at (e.g. -200 ms)','Epoch ends at (e.g. 2340 ms)',...
    'Sampling rate (e.g. 500 Hz)','Electrodes'};
dlg_title = 'Waveform properties';
num_lines = 1;
defAns = {'-100','500','500','5'};
...
[ch status] = str2num(answer{4});
if ~status  %%%Handle empty value returned for unsuccessful conversion
    msgbox('Invalid Number','Error in Parameter settings','error');
end
%% Get ERP waveforms
% select individual waveforms for std-O
[fname1,pathname1] = uigetfile(...
    { '*std-O.dat*','Select ERP waveform';'*.dat','All dat Files' }, ...
    'Select the first waveform', ...
    'Multiselect','on');
% Abort if the user hit 'Cancel'
if isequal(fname1,0)||isequal(pathname1,0),disp('Aborted.'); return; end

% select individual waveforms for std-X3
[fname2,pathname2] = uigetfile(...
    { '*std-X3.dat*','Select ERP waveform';'*.dat','All dat Files' }, ...
    'Select the first waveform', ...
    'Multiselect','on');
% Abort if the user hit 'Cancel'
if isequal(fname2,0)||isequal(pathname2,0),disp('Aborted.'); return; end

% select group averaged waveform
[fnameG,pathnameG] = uigetfile(...
    { '*std-X3.dat*','Select ERP waveform';'*.dat','All dat Files' }, ...
    'Select the first waveform', ...
    'Multiselect','off');
% Abort if the user hit 'Cancel'
if isequal(fnameG,0)||isequal(pathnameG,0),disp('Aborted.'); return; end
...
%% load the group averaged waveform
ffile = fullfile(pathnameG,fnameG);
...
for sj=1:length(fname1)
%read waveform of std-O
ffile = fullfile(pathname1,fname1{sj});
fprintf(1,'Processing %s\n',ffile);
[fid,msg] = fopen(ffile, 'rt'); %%% get file indentifier. Here 'r' sets the permission, which is the type of access of the file, read, write, append, or update. attching a 't' specifies whether to open files in binary or text mode.
if fid == -1,
    fprintf(1,'Error opening dat file "%s":', ffile);
...
% read waveform of std-X3
ffile = fullfile(pathname2,fname2{sj});
fprintf(1,'Processing %s\n',ffile);
[fid,msg] = fopen(ffile, 'rt'); %%% get file indentifier. Here 'r' sets the permission, which is the type of access of the file, read, write, append, or update. attching a 't' specifies whether to open files in binary or text mode.
if fid == -1,
    fprintf(1,'Error opening dat file "%s":', ffile);
...
%% waveform plotting
subplot(ceil(length(fname1)/2),2,sj)
ts = stt:1000/ft:endt;
...
for i = 1:length(ch)
    ...
    hold on
    plot(ts,wave2(1:xl,ch(i)),'--','color',rgb1(i,:),'LineWidth',2)
end
plot(ts,gwave(1:xl,ch(i)),'-','color',rgb1(4,:),'LineWidth',4)
...
saveas(fig,(['Gave-' fname1{1}(6:10) fname1{1}(18:end-4) '&' fname2{1}(6:10) fname2{1}(18:end-4) '-' chls{ch} '.png']));
saveas(fig,(['Gave-' fname1{1}(6:10) fname1{1}(18:end-4) '&' fname2{1}(6:10) fname2{1}(18:end-4) '-' chls{ch} '.fig']));
%% code ends here
