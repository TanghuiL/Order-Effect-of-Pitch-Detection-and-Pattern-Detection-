...
for i = 1:length(ch)
    ...
    max_w(i) = max(wave1(1:xl,ch(i)));
    min_w(i) = min(wave1(1:xl,ch(i)));
    hold on
end
plot(ts,gwave(1:xl,ch(i)),'-','color',rgb1(4,:),'LineWidth',4)
max_w(i+1) = max(gwave(1:xl,ch(i)));
min_w(i+1) = min(gwave(1:xl,ch(i)));
% if length(ch) == 1
...
xlim([-100 300]); xticks([-100 0 100 200 300])
ymax = max(max_w);ymin = min(min_w);
ylim([ymin ymax]);
% ylim([nyscale yscale]); 
...
saveas(fig,(['Gave-' fname{1}(6:10) fname{1}(18:end-4) '-' chls{ch} '.png']));
saveas(fig,(['Gave-' fname{1}(6:10) fname{1}(18:end-4) '-' chls{ch} '.fig']));
%% code ends here
